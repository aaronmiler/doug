...
- Every bash command already spawns in the correct working directory (see "Current working directory" at the end of this prompt) — never prefix a command with `cd <path> &&` to get there. It adds noise and, since {{name}}'s command allowlist matches the full command string, makes "always allow" stick to one path instead of the command itself
